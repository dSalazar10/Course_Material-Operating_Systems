 ...
 *      takes two args, each a command, and connects av[1]'s output to
 *      input of av[2]
...
int main( int ac, char* av[] )
{
    ...
    {
        close( thepipe[1] );                        // parent doesn't write

        if ( dup2( thepipe[0], 0 ) == -1 )
            oops( "Could not redirect stdin", 3 );

        close( thepipe[0] );                        // stdin duped, close pipe
        execlp( av[2], av[2], NULL );
        oops( av[2], 4 );
    }

    // child execs av[1] and writes into pipe
    close( thepipe[0] );                            // child doesn't read pipe

    if ( dup2( thepipe[1], 1 ) == -1 )
        oops( "could not redirect stdout", 4 );

    close( thepipe[1] );                            // stdout duped, close pipe
